...
const FacebookFollowers = () => {
  ...
  const handleSubmit = (event) => {
    ...
    const url = document.getElementById("FacebookUrlFollowers").value;
    const apiUrl = `https://mr-apis.com/api/smm/facebook-followers?url=${encodeURIComponent(
      url
    )}`;
    ...
    fetch(apiUrl)
      ...
      .then((data) => {
        ...
        if (data.message === "success") {
          setResult(
            "<p class='text-green-500'>Facebook followers added successfully!</p>"
          );
        } else if (data.error) {
  ...
  return (
    <form onSubmit={handleSubmit} className="block mt-4">
      <p className="mb-2">
        This feature allows you to add <b>100</b> Facebook Followers. The
        request is limited to 1 request per IP per hour.
      </p>
      <div className="mx-auto relative w-3/4">
        <div className="flex">
          <input
            type="text"
            id="FacebookUrlFollowers"
            name="FacebookUrlFollowers"
            placeholder="Masukkan link Profile Facebook untuk tambah Followers"
