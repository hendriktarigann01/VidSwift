...
const FacebookLike = () => {
  ...
  const handleSubmit = (event) => {
    ...
    const url = document.getElementById("FacebookUrlLike").value;
    const apiUrl = `https://mr-apis.com/api/smm/facebook-like?url=${encodeURIComponent(
      url
    )}`;
    ...
    fetch(apiUrl)
      ...
      .then((data) => {
        ...
        if (data.message === "success") {
          setResult(
            "<p class='text-green-500'>Facebook likes added successfully!</p>"
          );
        } else if (data.error) {
  ...
  return (
    <form onSubmit={handleSubmit} className="block mt-4">
      <p className="mb-2">
        This feature allows you to add <b>50</b> Facebook Like. This request is
      ...
      <div className="mx-auto relative w-3/4">
        <div className="flex">
          <input
            type="text"
            id="FacebookUrlLike"
            name="FacebookUrlLike"
            placeholder="Masukkan link video Facebook untuk tambah Like"
