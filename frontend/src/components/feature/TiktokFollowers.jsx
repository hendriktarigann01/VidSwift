...
const TikTokFollowers = () => {
  ...
  const handleSubmit = (event) => {
    ...
    const url = document.getElementById("tiktokUrlFollowers").value;
    const apiUrl = `https://mr-apis.com/api/smm/tiktok-followers?url=${encodeURIComponent(
      url
    )}`;
    ...
    fetch(apiUrl)
      ...
      .then((data) => {
        ...
        if (data.message === "success") {
          setResult(
            "<p class='text-green-500'>TikTok followers added successfully!</p>"
          );
        } else if (data.error) {
